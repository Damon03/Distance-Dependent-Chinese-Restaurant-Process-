...
sequential.example <- function ()
{
  ...
  docs <- read.documents("../data/corpus.txt")
  ...
  mat = read.table('../data/embeddings.txt')
  #mat = 5-mat
  #mat = 1+mat
  ...
  res <- ddcrp.gibbs(dat=dat, dist.fn=matrix.dist.fn(mat), alpha=0,
  ...
  write.table(clusters, file="../data/cluster.txt", row.names=FALSE, col.names=FALSE)
}
